...
class ERAN:
    def __init__(self, model=None, session=None, is_onnx=False, pkl_file=None, operations=None, resources=None, orig_input_shape=None):
        """
        ...
        """
        if operations is None or resources is None or orig_input_shape is None:
            assert model is not None, "Either operations and resources or model is required"
            if is_onnx:
                    ...
                    for j in ["deeppoly", "deepzono"]:
                        ...
                        for k in range(len(resources[i][j])):
                            if isinstance(resources[i][j][k], google.protobuf.pyext._message.RepeatedScalarContainer):
                                ...
                                res += (resources[i][j][k],)
                        resources[i][j] = res
                pkl.dump((resources, operations, orig_input_shape), open(pkl_file, "wb"))
        self.input_shape = resources[0]["deeppoly"][2]
        self.optimizer = Optimizer(operations, resources)
        self.orig_input_shape = orig_input_shape
    ...
    def analyze_box(self, specLB, specUB, domain, timeout_lp, timeout_milp, use_default_heuristic,
                    ...
                    eval_instance=None, verbosity=2, start_time=None, feas_act=None):
        """
        ...
        if domain == 'deepzono' or domain == 'refinezono':
            ...
            analyzer = Analyzer(execute_list, nn, domain, timeout_lp, timeout_milp, output_constraints,
                                ...
                                partial_milp=partial_milp, max_milp_neurons=max_milp_neurons, eval_instance=eval_instance, feas_act=feas_act)
            ...
            analyzer = Analyzer(execute_list, nn, domain, timeout_lp, timeout_milp, output_constraints,
                                ...
                                approx_k=approx_k, eval_instance=eval_instance, feas_act=feas_act)
        dominant_class, nlb, nub, failed_constraints, x, model_bounds = analyzer.analyze(terminate_on_failure=terminate_on_failure,
        ...
        if testing:
            ...
            return dominant_class, nn, nlb, nub, failed_constraints, x, model_bounds
    ...
    def analyze_zonotope(self, zonotope, domain, timeout_lp, timeout_milp, use_default_heuristic, output_constraints=None, testing = False, prop=-1):
        ...
        dominant_class, nlb, nub, failed_constraints, x, model_bounds = analyzer.analyze()
        return dominant_class, nn, nlb, nub, failed_constraints, x, model_bounds
