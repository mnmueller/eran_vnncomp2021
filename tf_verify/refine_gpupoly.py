...
import time
from utils import check_timeout, check_timeleft
...
def refine_gpupoly_results(nn, network, config, relu_layers, true_label, adv_labels=-1, K=3, s=-2,
    ...
    for l in relu_layers:
        ...
        if config.refine_neurons==True and not check_timeout(config, start_time, 0.2 * min(config.timeout_final_lp, config.timeout_final_milp)):
        ...
        if config.debug:
            print(f"total size: {total_size}, number of groups {len(kact_args)}, time since start: {time.time()-start_time}")
        ...
        for i_a in range((int)(np.ceil(A.shape[0] / max_eqn_per_call))):
            ...
            bounds_temp = network.evalAffineExpr(A_temp, layer=l-1, back_substitute=network.FULL_BACKSUBSTITUTION, dtype=np.double)
    ...
    if check_timeout(config, start_time, 1):
        return False, nn, nlb, nub, constraints, None, None
    ...
    if complete:
        ...
        model.setParam(GRB.Param.TimeLimit, check_timeleft(config, start_time, timeout_final_lp))
    ...
    if partial_milp != 0:
        counter_partial_milp, var_list_partial_milp, model_partial_milp = create_model(nn, nn.specLB, nn.specUB, nlb,
                                                                                       ...
                                                                                       feas_act=feas_act)
        model_partial_milp.setParam(GRB.Param.TimeLimit, check_timeleft(config, start_time, timeout_final_milp))
    ...
    # input_lb_adv, input_ub_adv = [], []
    # for or_list in constraints:
    #     lb_adv, ub_adv = refine_input_with_constr(model, var_list, counter, nn.specLB, nn.specUB, or_list, start_time, 1, 50, input_idxs=None)
    #     if lb_adv is None:
    #         if config.debug:
    #             print(f"Constraint {or_list} should be certified per spurious region guided refinement")
    #     input_lb_adv.append(lb_adv)
    #     input_ub_adv.append(ub_adv)
    #
    # if all([x is None for x in input_lb_adv]):
    #     if config.debug:
    #         print(f"This should be certified per spurious region guided refinement")
