...
def solver_call(ind):
    ...
    if Cache.terminate_time - time.time() < 1:
        return Cache.lbi[ind], Cache.ubi[ind], False, 0

    model = Cache.model.copy()
    model.setParam(GRB.Param.TimeLimit, max(0, min(Cache.time_limit, Cache.terminate_time - time.time())))
    runtime = 0
    ...
    for or_list in constraints:
        ...
        for is_greater_tuple in or_list:
            ...
            for cutoff in [0.01, 0.1, GRB.INFINITY]:
                model.reset()
                if (config.timeout_complete + start_time - time.time()) < 0.1:
                    continue
                model.setParam(GRB.Param.TimeLimit, check_timeleft(config, start_time, config.timeout_final_milp))
...
def evaluate_model(model, var_list, counter, input_len, is_greater_tuple, start_time, time_out, verbosity=2, callback=None):
    or_result = False
    sol = None
    model.reset()
    obj = obj_from_is_greater_tuple(is_greater_tuple, var_list, counter)
...
def evaluate_models(model_lp, var_list_lp, counter_lp, input_len, constraints, terminate_on_failure, model_p_milp=None,
                    ...
                    start_time=None, prio_adex=False):
    ...
    for _ in range(len(constraints)):
        ...
        for is_greater_tuple in or_list:
            ...
            or_result, adex, obj_bound = evaluate_model(model_lp, var_list_lp, counter_lp, input_len, is_greater_tuple, start_time, config.timeout_final_lp, verbosity)
        ...
        if find_adex and not or_result and not adex_found and len(constraints)==1 and len(constraints[0])>1 and eval_instance is not None:
            if check_timeout(config, start_time, 0.1):
                continue
            _, adex, _ = evaluate_model(model_lp, var_list_lp, counter_lp, input_len, constraints[0], start_time, config.timeout_final_lp, verbosity)
            if adex is not None:
                if eval_instance is not None:
                    cex_label, cex_out = eval_instance(adex)
                    adex_found = not evaluate_cstr(constraints, cex_out)
        ...
        if model_p_milp is not None and (not or_result) and (not adex_found):
            ...
            if len(or_list) > 1 and not check_timeout(config, start_time, 0.1) and len(constraints) == 1 and len(constraints[0]) > 1 and eval_instance is not None:
            ...
            if (not or_result) and (not adex_found):
                for is_greater_tuple in or_list:
                    ...
                    or_result, adex, obj_bound = evaluate_model(model_p_milp, var_list_p_milp, counter_p_milp, input_len,
                                                                ...
                                                                verbosity, callback=(milp_callback_adex if find_adex else milp_callback))
            ...
            if find_adex and len(or_list)==1 and (not or_result) and (not adex_found) and len(constraints) == 1 and len(constraints[0]) > 1 and eval_instance is not None:
                if check_timeout(config, start_time, 0.1):
                    continue
                _, adex, _ = evaluate_model(model_p_milp, var_list_p_milp, counter_p_milp, input_len, constraints[0],
                                            start_time, config.timeout_final_milp, verbosity, )
...
#             else:
#                 model_lp.setParam(GRB.Param.TimeLimit, check_timeleft(config,start_time,config.timeout_final_lp))
#                 model_lp.optimize()
...
#                 adex = model_lp.x[0:input_len]
#                 if eval_instance is not None:
#                     cex_label, cex_out = eval_instance(adex)
#                     adex_found = not evaluate_cstr(constraints, cex_out)
#                 if adex_found or eval_instance is None:
#                     adex_list_or.append(adex)
...
#                 continue
#             model_lp.reset()
#             obj = obj_from_is_greater_tuple(constraints[0], var_list_lp, counter_lp)
#             model_lp.setObjective(obj, GRB.MINIMIZE)
#             model_lp.setParam(GRB.Param.TimeLimit, check_timeleft(config, start_time, config.timeout_final_lp))
#             model_lp.optimize()
#
#             if verbosity>=2:
#                 print(f"Adv model status: {model_lp.Status}, Obj count: {model_lp.SolCount}, Final solve time: {model_lp.Runtime:.3f}")
#
#             if model_lp.SolCount > 0:
#                 adex = model_lp.x[0:input_len]
#                 cex_label, cex_out = eval_instance(adex)
#                 adex_found = not evaluate_cstr(constraints, cex_out)
#                 if adex_found: adex_list.append(adex)
#
#         ### do partial milp for elements in or list
#         if not or_result and not adex_found and model_p_milp is not None:
#             for is_greater_tuple in or_list:
#                 if not check_timeout(config, start_time, 0.1) and not adex_found:
#                     model_p_milp.reset()
#                     obj = obj_from_is_greater_tuple(is_greater_tuple, var_list_p_milp, counter_p_milp)
#                     model_p_milp.setObjective(obj, GRB.MINIMIZE)
#                     model_p_milp.setParam(GRB.Param.TimeLimit, check_timeleft(config, start_time, config.timeout_final_milp))
#                     if find_adex:
#                         model_p_milp.optimize(milp_callback_adex)
#                     else:
...
#                         adex = model_p_milp.x[0:input_len]
#                         if eval_instance is not None:
#                             cex_label, cex_out = eval_instance(adex)
#                             adex_found = not evaluate_cstr(constraints, cex_out)
#                         if adex_found or eval_instance is None:
...
#                     continue
#                 model_p_milp.reset()
#                 obj = obj_from_is_greater_tuple(constraints[0], var_list_p_milp, counter_p_milp)
#                 model_p_milp.setObjective(obj, GRB.MINIMIZE)
#                 model_p_milp.setParam(GRB.Param.TimeLimit, check_timeleft(config, start_time, config.timeout_final_milp))
#                 model_p_milp.optimize()
#
#                 if verbosity >= 2:
#                     print(
#                         f"Adv partial milp model status: {model_p_milp.Status}, Obj count: {model_p_milp.SolCount}, Final solve time: {model_p_milp.Runtime:.3f}")
#
#                 if model_p_milp.SolCount > 0:
#                     adex = model_p_milp.x[0:input_len]
#                     cex_label, cex_out = eval_instance(adex)
#                     adex_found = not evaluate_cstr(constraints, cex_out)
#                     if adex_found: adex_list.append(adex)
